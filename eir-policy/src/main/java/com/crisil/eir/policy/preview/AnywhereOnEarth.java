 ...
 * {@code PolicyVersion.approvedOn} and {@link ImpactPreview#portfolioAsOf()} are dates, because
 * that is how an approval and a book position are recorded, while
 * {@link ImpactPreview#generatedAt()} is an instant, because a preview's age has to be
 * measurable to better than a day. Comparing them requires a zone that neither record carries.
 *
...
final class AnywhereOnEarth {
     ...
     */
    static Instant earliestInstantOf(LocalDate date) {
     ...
     */
    static Instant firstInstantAfter(LocalDate date) {
        Objects.requireNonNull(date, "date");
